...
// apply.)
export type Budget = {|
  +lines: $ReadOnlyMap<NodeAddressT, $ReadOnlyArray<BudgetPolicy>>,
...
export function applyBudget(
  ...
  if (anyCommonPrefixes(Array.from(budget.lines.keys()))) {
  ...
  for (const [prefix, policies] of budget.lines) {
    ...
    let policyIndex = -1;
    let activeBudget = Infinity;
    for (const {interval, nodes} of weeklyGraphPartition) {
      while (
        // Is there another policy we could switch to
        policyIndex + 1 < policies.length &&
        // Is that next policy in effect as of this interval
        policies[policyIndex + 1].startTimeMs <= interval.startTimeMs
      ) {
        // Then increment policyIndex and update the active budget
        policyIndex++;
        activeBudget = policies[policyIndex].budget;
      }
      let mintedCredInInterval = 0;
      for (const {address} of nodes) {
        mintedCredInInterval += evaluator(address);
      }
      if (mintedCredInInterval > activeBudget) {
        const normalizer = activeBudget / mintedCredInInterval;
        for (const {address} of nodes) {
          if (evaluator(address) === 0) {
            // This node already has 0 weight, so no reason to set a
            // normalizer.
            continue;
          }
